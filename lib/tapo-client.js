...
// Module-level hash helpers — defined once, not re-created on every authenticate() call.
...
// Single-pass replace: colon → ": ", comma → ", "
const pyJson  = o => JSON.stringify(o).replace(/:|,/g, m => m === ':' ? ': ' : ', ');
...
function post(agent, ip, path, body) {
    return new Promise((resolve, reject) => {
        ...
        const req  = https.request({
            ...
            res.on('end', () => { const s = Buffer.concat(chunks).toString(); try { resolve(JSON.parse(s)); } catch(e) { reject(new Error('Bad JSON from camera: ' + s.slice(0, 200))); } });
        });
...
// body must be a Buffer — callers pre-compute it to avoid double encoding.
function postRaw(agent, ip, path, body, headers) {
    return new Promise((resolve, reject) => {
        const req = https.request({
...
async function authenticate(ip, username, password) {
    ...
    try {
        return await _authenticate(ip, username, password, agent);
    } catch (err) {
        // Any failure between agent creation and return would otherwise leak the
        // HTTPS agent (with keep-alive sockets) until GC eventually collects it.
        // On an offline camera this happens on EVERY retry — a slow but real
        // memory leak.  Destroy the agent now so sockets are closed immediately.
        agent.destroy();
        throw err;
    }
}

async function _authenticate(ip, username, password, agent) {
    ...
    // Camera returns locked status either as {result:{data:{code:-40404}}} or {data:{code:-40404},error_code:-40401}
    const r1Data = r1?.result?.data ?? r1?.data;
    if (r1Data?.code === -40404 || r1?.error_code === -40401) {
        throw Object.assign(new Error(`Account locked — wait ${r1Data?.sec_left ?? 0}s`), { code: 'LOCKED' });
    }
    ...
    const r2Data = r2?.result?.data ?? r2?.data;
    if (r2Data?.code === -40404 || r2?.error_code === -40401) {
        throw Object.assign(new Error(`Account locked — wait ${r2Data?.sec_left ?? 0}s`), { code: 'LOCKED' });
    }
    ...
    // Pre-built headers object — static fields set once per session, only 3 dynamic
    // fields mutated per command. Safe because commands are serialized by the queue.
    const _cmdHeaders = {
    ...
    async function sendCmd(apiCmd) {
        ...
        let r;
        try {
            _cmdHeaders['Content-Length'] = buf.length;
            _cmdHeaders['Seq']            = String(curSeq);
            _cmdHeaders['Tapo_tag']       = tag;
            r = await postRaw(agent, ip, `/stok=${stok}/ds`, buf, _cmdHeaders);
        } catch (err) {
...
const _queues        = new Map();       // `${ip}|${user}` → { tail: Promise, depth: number }
// Max commands queued per device while camera is unreachable.
// Prevents unbounded Promise-chain growth when flows keep firing against an offline camera.
...
async function executeOnDevice(ip, username, password, apiCmd) {
    ...
    const entry = _queues.get(key);

    // Reject immediately if too many commands are already queued — prevents
    // unbounded Promise-chain growth when the camera is offline and flows
    // keep firing (every 12 s timeout × N pending = minutes of backlog on Pi 3).
    if (entry && entry.depth >= QUEUE_MAX_DEPTH) {
        return Promise.reject(new Error(`Device ${ip} queue full (${QUEUE_MAX_DEPTH} pending) — camera may be offline`));
    }
    ...
    const prevOp = (entry ? entry.tail : Promise.resolve()).catch(() => {});
    const depth  = entry ? entry.depth + 1 : 1;
    ...
    const thisOp = prevOp.then(async () => {
        ...
        if (!cached || Date.now() >= cached.expiresAt) {
            ...
            let authResult;
            try {
                authResult = await authenticate(ip, username, password);
            } catch (authErr) {
                // SSL error during auth — C225 firmware may not have fully freed its
                // single connection slot yet (e.g. after ONVIF stop).  Wait and retry once.
                if (/ERR_SSL_WRONG_VERSION_NUMBER|ssl3_get_record|wrong version number|packet length too long|EPROTO/i.test(authErr.message)) {
            ...
            cached = { sendCmd: authResult.sendCmd, agent: authResult.agent, expiresAt: Date.now() + SESSION_TTL_MS };
        ...
        try {
            ...
            // SSL error — stale keep-alive socket or camera slot not yet free.
            // Destroy the session, wait, and retry once with a fresh TLS handshake.
            if (/ERR_SSL_WRONG_VERSION_NUMBER|ssl3_get_record|wrong version number|packet length too long|EPROTO/i.test(err.message)) {
    ...
    // Store queue entry; clean up when this op finishes so the Map doesn't grow forever.
    const tail = thisOp.catch(() => {});
    _queues.set(key, { tail, depth });
    tail.finally(() => {
        // Only delete if this is still the tail (no newer op registered after us).
        const cur = _queues.get(key);
        if (cur && cur.tail === tail) _queues.delete(key);
    });
    return thisOp;
}
...
// Static parts of the detection search command — hoisted to avoid per-call object allocation.
const _DETECT_ACCEPT_ERRORS = Object.freeze([-64303, -40105, -71114]);
...
async function getDetections(ip, username, password, minutes) {
    ...
    const apiCmd = {
        ...
        _acceptErrors: _DETECT_ACCEPT_ERRORS,
    };
    const result = await executeOnDevice(ip, username, password, apiCmd);
    return result?.playback?.search_detection_list ?? [];
}

// Called by tapo-ptz before sending a PTZ SOAP command.
// If tapo-onvif-events is running it aborts the in-flight PullMessages so the
// camera's single connection slot is free immediately (otherwise PTZ would wait
// up to pullTimeoutSec seconds for the hold to expire).
async function interruptOnvifForPtz(ip) {
    const reg = _onvifRegistry.get(ip);
    if (reg && reg.interrupt) await reg.interrupt();
}

// Release (destroy) a cached session's HTTPS agent, immediately freeing any
// idle keep-alive socket.  Called by tapo-onvif-events before ONVIF operations
// so the camera's single TCP connection slot is available for SOAP calls.
// Safe to call even if no session exists (no-op in that case).
function releaseSession(ip, username) {
    const key    = `${ip}|${username}`;
    const cached = _sessions.get(key);
    if (cached) { cached.agent.destroy(); _sessions.delete(key); }
}

module.exports = { authenticate, executeOnDevice, COMMANDS, getDetections, registerOnvifCallbacks, unregisterOnvifCallbacks, interruptOnvifForPtz, releaseSession };
