...
async function executeOnDevice(ip, username, password, apiCmd) {
    ...
    const thisOp = prevOp.then(async () => {
        ...
        if (reg && privacyOn) await reg.stop();
...
const COMMANDS = {
    ...
    // ── Package detection ───────────────────────────────────────────────────────
    'package-on':  { method: 'setPackageDetectionConfig', params: { package_detection: { detection: { enabled: 'on'  } } } },
    'package-off': { method: 'setPackageDetectionConfig', params: { package_detection: { detection: { enabled: 'off' } } } },
